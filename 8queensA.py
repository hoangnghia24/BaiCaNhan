...
def an_toan(state, col,row):
    for r, c in enumerate(state):
        if c == col or abs(c - col) == abs(r - row):
            return False
    return True
def gn(cost,state,col,row):
    '''chi phí từ vị trí ban đầu đến hiện tại'''
    if not state:
        return cost
    for r, c in enumerate(state):
        if r == len(state) - 1:
...
def hn(state,row):
    ...
    for r in range(row, 8):
        for c in range(8):
            if cols[c] or diag1[r + c] or diag2[r - c + 7]:
                count += 1
    return count
def fn(cost,new_state,col,row,state):
    return hn(new_state,row+1) + gn(cost,state,col,row)
def A():
    q = [(0, [])]
    solutions = []
    while q:
        ...
        for col in range(8):
            if an_toan(state, col, row):
                new_state = state + [col]
                cost = fn(cost,new_state,col,row,state)
                heapq.heappush(q, (cost, new_state))
    return solutions
solutions = A()
